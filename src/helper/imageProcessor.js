...
// Cache to track which images have been processed
 ...
 */
export async function processImage(sourcePath, outputDir, relativeDir) {
...
export async function processAllImages(imageFiles, sourceDir, outputDir, progressCallback) {
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    const result = await processImage(file, absoluteOutputDir, relativeDir);
    if (result) {
      // Store with leading slash for easy lookup
